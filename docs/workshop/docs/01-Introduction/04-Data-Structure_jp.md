...
アクセラレータは、**作業範囲記述書（SOW）、請求書、検証結果、抽出されたエンティティ**を構造化されたPostgreSQLスキーマを使用して処理します。以下は、**主要なテーブル**とその役割の概要です：
...
| `sows`                      | 作業範囲記述書に関する情報を含み、条件、関係者、成果物を含みます。 |
...
|`sow_id`,`sow_number`       |    `vendor_graph`       |    作業範囲記述書 (SOW) を追跡し、検証するために使用されます    |
